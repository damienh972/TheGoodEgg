    ...
    async updateBurnedAndUnrevealed(leaderboard) {
      ...
      try {
        ...
        const response = await fetch(
          'https://subgraph.satsuma-prod.com/eaa3f9356941/damien-test--19534/thegoodegg/api',
          {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json'
        ...
        if (newBurnedIds.length > 0) {
          ...
          this.unrevealed = this.unrevealed.filter((egg) => !newBurnedIds.includes(egg.tokenId))
        }
      } catch (error) {
        console.log(error)
        this.error = error.message
