      ...
      return apiResponses
        .map((response) => {
          if (
            ...
            const species = attributes.find((attr) => attr.trait_type === 'Species')?.value || 'missing_metadata';
            const generation = attributes.find((attr) => attr.trait_type === 'Generation')?.value || 'missing_metadata';
            const aura = attributes.find((attr) => attr.trait_type === 'Aura')?.value || 'missing_metadata';
            const element = attributes.find((attr) => attr.trait_type === 'Element')?.value || 'missing_metadata';
            const type = attributes.find((attr) => attr.trait_type === 'Type')?.value || 'missing_metadata';
